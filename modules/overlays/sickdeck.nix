{inputs, ...}: {
  flake.overlays.sickdeck = final: prev: let
    version = "0.1.33";
    ...
    server = final.rustPlatform.buildRustPackage {
      ...
      sourceRoot = "${root}/packages/server";
    ...
    client = final.buildNpmPackage {
      pname = "sickdeck-client";
      inherit version src;

      sourceRoot = "${root}/packages/client";
      npmDepsHash = "sha256-Kc1i6iTwzK9IAckrRBy8BnqWyZzqNWJVRJtHklu6vhM=";
    ...
    sickdeckTest = final.stdenvNoCC.mkDerivation {
      pname = "sickdeck-test";
      inherit version src;

      sourceRoot = "${root}/packages/sickdeck-test";
  ...
  in {
    sickdeck = final.stdenvNoCC.mkDerivation {
      ...
      installPhase = ''
        ...
        install -m755 ${server}/bin/sickdeck-server "$root/build/sickdeck-bin"
        ln -s sickdeck-bin "$root/build/sickdeck-bin-darwin-arm64"

        makeWrapper ${final.nodejs_26}/bin/node $out/bin/sickdeck \
          --add-flags "$root/packages/cli/bin/sickdeck.mjs"

        mkdir -p "$skill"
        ${server}/bin/sickdeck-server skill-md > "$skill/SKILL.md"
